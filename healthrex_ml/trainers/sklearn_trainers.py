...
class NGBoostTrainer():
    """
    Trains a NGBoost and performs appropriate model selection. 
    """
    ...
    def __call__(self, task):
        ...
        # Fit model with early stopping
        self.ngb.fit(X_train, y_train[self.task].values)

        # Predictions
        Y_pred = self.ngb.predict(X_test)
        Y_dists = self.ngb.pred_dist(X_test)
