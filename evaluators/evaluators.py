...
class NPVEvaluator:
    ...
    def __call__(self, labels, predictions, lim_threshold=0.1):
        """
        Override in child classes to include functionality to pull labels and
        predictions from some outside source (ex: cosmos db)
        """
        y_mean = np.asarray(predictions[0])
        y_dist = np.asarray(predictions[1])
        cur_measure = np.asarray(labels[0])
        last_measure = np.asarray(labels[1])
        upper_lim = last_measure * (1 + lim_threshold)
        lower_lim = last_measure * (1 - lim_threshold)
        binary_labels = ((cur_measure>=lower_lim)&(cur_measure<=upper_lim))
        stable_prob = 0.5 * (scipy.special.erf((upper_lim - y_mean)/(math.sqrt(2)*y_dist)) - scipy.special.erf((lower_lim - y_mean)/(math.sqrt(2)*y_dist)))
        self.get_performance_artifacts(binary_labels, stable_prob)
